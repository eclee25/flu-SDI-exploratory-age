...
WHERE flu.SERVICE_PLACE = "TOTAL" and child.MARKER <> 'O' and flu.patient_zip3 <> 'TOT'
GROUP BY season.SEASON_NUM, flu.WEEK, flu.patient_zip3, child.MARKER
INTO OUTFILE '/tmp/OR_zip3_week.csv'
...
from flu RIGHT JOIN season ON (flu.WEEK = season.WEEK) RIGHT JOIN child ON (child.AGEGROUP = flu.AGEGROUP)
WHERE flu.SERVICE_PLACE = "TOTAL" and child.MARKER <> 'O' and flu.patient_zip3 = '753' and season.SEASON_NUM = 2
