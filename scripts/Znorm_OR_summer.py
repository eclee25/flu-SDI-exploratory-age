...
###Date: 1/8/14
###Function: norm by weeks 21 to 39 (summer weeks)
...
###Import data: SDI_Data/explore/SQL_export/OR_allweeks.csv, SDI_Data/explore/SQL_export/OR_allweeks_office.csv

###Command Line: python Znorm_OR_summer.py
##############################################
...
for s in seasons:
	...
	wkdummy = set(wkdummy)
	# normwkdummy will represent summer wks prior to flu season (wks 21 to 39 inclusive)
	normwkdummy = [key for key in sorted(weeks) if wkdict[key] == int(s) - 1 and key.isocalendar()[1] > 20 and key.isocalendar()[1] < 40]
	normwkdummy = set(normwkdummy)
	# all data
	s_mean = np.mean([ORdict[wk] for wk in normwkdummy])
	s_sd = np.std([ORdict[wk] for wk in normwkdummy])
	dictdummyls = [(ORdict[wk]-s_mean)/s_sd for wk in sorted(wkdummy)]
	print 'all data: s, s_mean, s_sd:', s, s_mean, s_sd
	for w, z in zip(sorted(wkdummy), dictdummyls):
		ORdict_znorm[w] = z
	# offices/op data
	s_mean2 = np.mean([ORdict2[wk] for wk in normwkdummy])
	s_sd2 = np.std([ORdict2[wk] for wk in normwkdummy])
...
plt.ylabel('z-normalized (summer months) OR, child:adult', fontsize=24)
...
plt.xlim([0, 52])
plt.ylim([-10, 15])
plt.xlabel('Week Number, offices/OP only', fontsize=24) # 12/1/13 increase size
plt.ylabel('z-normalized OR (summer months), c:a', fontsize=24)
plt.legend(loc = 'upper left')
plt.xticks(xrange(53), xlabels)
plt.show()
