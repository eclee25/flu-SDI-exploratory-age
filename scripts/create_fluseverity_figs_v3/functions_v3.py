...
## v3: continue swap, adjust incidence for age-specific HC seeking behavior, 
...
# weighted averages based on sample size in Biggerstaff2012 and Biggerstaff2014
# children = 5-17, adults = 18-64. See health_seeking_behavior_edited_101014.ods
dict_careseek_nat = {'C':0.5146, 'A':0.4095} 
...
def coverageCareseek_adjustment(filename_anydiagAge, dict_ILI_week):
	''' Import any diagnosis visits by week and age group. Function argument is the filename string. Return two dicts: dict_anyvisit_week[(week, age group code)] = number of any diagnosis visits; dict_incidProp_week[(week, age group code)] = ILI cases * (any diagnosis in equivalent week in season 9)/(any diagnosis in week)
	'''
	...
	dict_anyvisit_season, dict_ili_season = defaultdict(list), defaultdict(list)
	for s in pseasons:
		dummyweeks = sorted([wk for wk in dict_wk if dict_wk[wk][0] == s])
		cVisits = [dict_anyvisit_week[(wk, 'C')] for wk in dummyweeks]
		aVisits = [dict_anyvisit_week[(wk, 'A')] for wk in dummyweeks]
		cILI = [dict_ILI_week[(wk, 'C')] for wk in dummyweeks]
		aILI = [dict_ILI_week[(wk, 'A')] for wk in dummyweeks]
		if len(dummyweeks) == 52:
			cVisits.insert(13, (cVisits[12]+cVisits[13])/2.)
			aVisits.insert(13, (aVisits[12]+aVisits[13])/2.)
			cILI.insert(13, (cILI[12]+cILI[13])/2.)
			aILI.insert(13, (aILI[12]+aILI[13])/2.)
		dict_anyvisit_season[(s, 'C')] = cVisits
		dict_anyvisit_season[(s, 'A')] = aVisits
		dict_ili_season[(s, 'C')] = cILI
		dict_ili_season[(s, 'A')] = aILI


	dict_covSeekAdjust_season = defaultdict(list)
	for key in dict_ili_season:
		s = key[0] # season number 
		age = key[1]
		careseek = dict_careseek_nat[age]
		iliDummy = dict_ili_season[key]
		visit9 = dict_anyvisit_season[(9, age)]
		visitS = dict_anyvisit_season[key]
		# adjust ILI by coverage level in comparable week in 08-09 season and care seeking behavior in that age group
		# dict_covSeekAdjust_season[key] = [ili*v9/vS/careseek for ili, v9, vS in zip(iliDummy, visit9, visitS)]
		dict_covSeekAdjust_season[key] = [ili*v9/vS/careseek for ili, v9, vS in zip(iliDummy, visit9, visitS)]

	return dict_covSeekAdjust_season

##############################################
def week_OR_processing(csv_incidence, csv_population):
	''' Import SQL_export/OR_allweeks_outpatient.csv data (or other OR_allweeks...csv data), which includes season number, week, age group, and ILI incid. Import SQL_export/totalpop_age.csv data, which includes calendar year, age group, and US population. Function 'coverageCareseek_adjustment' is nested. Return four dicts:
	...
	dict_adjILI_season = coverageCareseek_adjustment(fname, dict_ILI_week)

	# import total incidence by week (no adjustments)
	dict_incid = {}
	for wk in dict_wk:
		s = dict_wk[wk]
		tot_incid = sum([dict_ILI_week[(wk, age)] for age in age_keys]) / sum([dict_pop[(s, age)] for age in age_keys]) * 100000
	...
	# generate adjusted incidence per 100,000 in US population and OR at the weekly level
	for s in pseasons:
		# incid53ls dict
		incid53dummy = [dict_incid[wk] for wk in sorted(dict_wk) if dict_wk[wk] == s]
		if len(incid53dummy) == 52:
			incid53dummy.insert(13, (incid53dummy[12]+incid53dummy[13])/2.)
		dict_incid53ls[s] = incid53dummy
		# OR53ls dict
		child_attack = [adjILI/dict_pop[(s, 'C')] for adjILI in dict_adjILI_season[(s, 'C')]]
		adult_attack = [adjILI/dict_pop[(s, 'A')] for adjILI in dict_adjILI_season[(s, 'A')]]
		# 6/24/14: OR cannot be evaluated if child or adult incidence is zero. Assign OR = NA if one of those conditions is met.
		# OR = [(c/(1-c))/(a/(1-a)) if c and a else float('nan') for c, a in zip(child_attack, adult_attack)]
		OR = [a/c if c and a else float('nan') for c, a in zip(child_attack, adult_attack)] # 10/16/14 relative risk
		dict_OR53ls[s] = OR
		# zOR53ls dict
		normalization_period = dict_OR53ls[s][:gp_normweeks]
		season_mean = np.mean(normalization_period)
		season_sd = np.std(normalization_period)
		dict_zOR53ls[s] = [(val-season_mean)/season_sd for val in dict_OR53ls[s]]
...
def week_OR_processing_region(csv_incidence_region, csv_population_region):
	...
	for wk, r in product(dict_wk_sub, gp_plotting_regions):
		...
		dict_OR_reg[(wk, r)] = 1/float(OR) # swap age
...
def week_OR_processing_state(csv_incidence_region, csv_population_region):
	...
	for wk, state in product(dict_wk_sub, state_keys):
		...
		if not child_attack or not adult_attack:
			...
			OR = (child_attack/(1-child_attack))/(adult_attack/(1-adult_attack))	
		# dict_OR_state[(week, state)] = OR
		dict_OR_state[(wk, state)] = 1/float(OR) # 10/14/14 swap age
