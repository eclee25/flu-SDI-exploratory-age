...
print '%s of %s states have early and retro classif for all %s seasons' %(ct, len(states), len(seasons))
...
# list of states with all available data
incl_states = [st for st in d_st_match_early]
# d_correct_by_season_[seasonnum] = (# correct states early warning, # correct states retrospective)
for i in range(len(seasons)):
	d_correct_by_season[i+2] = (sum([d_st_match_early[st][i] for st in incl_states]), sum([d_st_match_retro[st][i] for st in incl_states]), sum([d_st_match_st2st[st][i] for st in incl_states]))
